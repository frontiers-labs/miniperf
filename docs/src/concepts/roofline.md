...
**Throughput.** Operations come from the accounting run, which counts every executed floating-point instruction per loop, scalar and vector, single and double precision. For RISC-V vector code the count uses the runtime `vl`, `vstart`, SEW, and mask state of each instruction. Time comes from the native run: PMU samples are matched to the loop's address ranges, and the loop's duration is the sampled share of the run. A loop gets a plotted point only when the estimated 95 % error of that timing is at most 10 %.
