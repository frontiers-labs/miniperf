...
//!
//! This crate exists to be a build-dependency of the vendored `libduckdb-sys`.
//! Cargo builds a build-dependency, running its build script, before it
//! compiles the build script that uses it -- so this is the one hook that runs
//! before `libduckdb-sys` is compiled and rustc bundles the static archive
//! named by `DUCKDB_LIB_DIR` into its rlib. A build script on any dependent of
//! `libduckdb-sys`, such as `miniperf-store`, runs far too late.
...
fn main() {
    let workspace = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").expect("Cargo sets it"))
        .ancestors()
        .nth(2)
        .expect("the crate lives at third_party/duckdb-prefetch")
        .to_path_buf();
